          ...
          if tp.Visible then
            ...
            if not getgenv().Multiplayer then
              if readfile(fileprefix.."PrivateServer.txt") ~= "PRIVATE SERVER LINK HERE" then
                ...
                if executorName and (string.find(executorName(), "ScriptWare") or executorName() == "ScriptWare") then
...
function scrip:Option(id, wave, name, value, timem, times, inbetween)
	spawn(function()
		appendfile(fileprefix.."LastStrat_"..PlayerUserId..".txt","TDS:Option("..id..", "..wave..", "..name..", "..value..", "..timem..", "..times..", "..tostring(inbetween or "false")..")\n")
		if isgame() then
			if not inbetween then
				printall("Setting option normal")
				repeat task.wait() until tonumber(stateRep:GetAttribute("Wave")) == wave and RS.State.Timer.Time.Value == conv(getDecimalsBef(times), timem) or tonumber(stateRep:GetAttribute("Wave")) == wave and (RS.State.Timer.Time.Value + 1) == conv(getDecimalsBef(times), timem) or getgenv().AbortThisShit
				printall("Setting option normal")
			else
				printall("Setting option between")
				getHalfTime(wave,times)
				printall("Set option between")
			end
			addSupposed()
			printall("Checking troop")
			repeat task.wait() until game.Workspace.Towers:FindFirstChild(tostring(id)) or getgenv().AbortThisShit
			...
			RSRF:InvokeServer("Troops","Option","Set",{["Troop"] = game.Workspace.Towers:WaitForChild(tostring(id)), ["Name"] = name, ["Value"] = value})
			addDidActs()
			getgenv().output("Set option")
		end
	end)
end
