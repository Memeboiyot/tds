...
local MainLink = LoadLocal and ''
    or 'https://raw.githubusercontent.com/Memeboiyot/tds/main/TDS/'
...
StratXLibrary.UtilitiesConfig = {
    ...
    PreferMatchmaking = getgenv().PreferMatchmaking
        or getgenv().Matchmaking
        or false,
    Webhook = {
        Enabled = true,
        Link = if (
                getgenv().WebhookLink
                and tostring(getgenv().WebhookLink) ~= nil
            )
...
local RemoteFunction = if not GameSpoof
    then ReplicatedStorage:WaitForChild('RemoteFunction')
    else SpoofEvent
local RemoteEvent = if not GameSpoof
    then ReplicatedStorage:WaitForChild('RemoteEvent')
    else SpoofEvent
...
local UILibrary = getgenv().UILibrary
    or loadstring(
        game:HttpGet(
            'https://raw.githubusercontent.com/Sigmanic/ROBLOX/main/WallyUI.lua',
            true
        )
    )()
...
local Patcher =
    loadstring(game:HttpGet(MainLink .. 'TDSTools/ConvertFunc.lua', true))() --loadstring(game:HttpGet("https://raw.githubusercontent.com/Sigmanic/Strategies-X/main/ConvertFunc.lua", true))()
...
if isfile('StrategiesX/TDS/UserConfig/UtilitiesConfig.txt') then
    local Check, GetConfig = pcall(function()
        return game:GetService('HttpService'):JSONDecode(
            readfile('StrategiesX/TDS/UserConfig/UtilitiesConfig.txt')
        )
    end)
    if Check then
    ...
    if
        tonumber(getgenv().DefaultCam)
        and tonumber(getgenv().DefaultCam) <= 3
    then
    ...
    if
        getgenv().WebhookLink
        and tostring(getgenv().WebhookLink) ~= nil
        and type(tostring(getgenv().WebhookLink)) == 'string'
    then
    ...
    if
        type(getgenv().PreferMatchmaking) == 'boolean'
        or type(getgenv().Matchmaking) == 'boolean'
    then
        UtilitiesConfig.PreferMatchmaking = getgenv().PreferMatchmaking
            or getgenv().Matchmaking
    end
else
    writefile(
        'StrategiesX/TDS/UserConfig/UtilitiesConfig.txt',
        game:GetService('HttpService'):JSONEncode(UtilitiesConfig)
    )
end
...
function SaveUtilitiesConfig()
    ...
    StratXLibrary.UtilitiesConfig = {
        ...
        Webhook = {
            ...
            Link = (#WebSetting.flags.Link ~= 0 and WebSetting.flags.Link)
                or if (
                        getgenv().WebhookLink
                        and tostring(getgenv().WebhookLink) ~= nil
                    )
            ...
            DisableCustomLog = if type(WebSetting.flags.DisableCustomLog)
                    == 'boolean'
                then WebSetting.flags.DisableCustomLog
                else true,
        },
    }
    UtilitiesConfig = StratXLibrary.UtilitiesConfig
...
function TowersCheckHandler(...)
    ...
    for i, v in next, { ... } do
        ...
        if
            not (
                TowersContained[Id]
                and typeof(TowersContained[Id].Instance) == 'Instance'
            )
        then
            task.delay(
                45,
                function() --game has wave 0 now so increase it to make it works
                    SkipTowerCheck = true
                end
            )
            if
                CurrentCount == StratXLibrary.RestartCount
                and not TowersContained[Id]
            then
            ...
            if
                (
                    CurrentCount == StratXLibrary.RestartCount
                    and TowersContained[Id]
                    and TowersContained[Id].Placed == false
                ) and not SkipTowerCheck
            then
                task.delay(2, function()
                    if
                        not (
                            CurrentCount == StratXLibrary.RestartCount
                            and TowersContained[Id].Instance
                            and TowersContained[Id].Placed
                        )
                    then
                        ConsoleWarn(
                            `Tower ID: {Id}, Type: \"{TowersContained[Id].TowerName}\" Hasn't Been Placed Yet. Waiting It To Be Placed`
                        )
                    end
                end)
                repeat
            ...
            if SkipTowerCheck then
                ...
                ConsoleWarn(
                    `Tower ID: {Id}, Table {TowersContained[Id]}, Instance: {TowersContained[Id] and TowersContained[Id].Instance or 'Nil'}`
                )
                return false
            end
        end
...
function TimeWaveWait(Wave, Min, Sec, InWave, Debug)
    local GameWave = LocalPlayer.PlayerGui
    ...
    task.spawn(function()
        ...
        if Total == 1 then
            ...
            if not ActionInfoTable[String][3] then
                ActionInfoTable[String][3] =
                    UI.ActInfo:Section(`{String} : 0 / 1`)
            end
        elseif Total > 1 and not ActionInfoTable[String][3] then
            repeat
                task.wait()
            until ActionInfoTable[String][3]
        end
...
function GetTowersInfo()
    ...
    repeat
        task.wait()
        GetResult =
            RemoteFunction:InvokeServer('Session', 'Search', 'Inventory.Troops')
    ...
    if not (UtilitiesConfig.BypassGroup or IsPlayerInGroup) then
        if CheckPlace() then
            ...
            JoinButton:Button('Copy Link Group', function()
                setclipboard(
                    'https://www.roblox.com/communities/4914494/Paradoxum-Games'
                )
            end)
...
if CheckPlace() then
    ...
    local RSHealthCurrent = ReplicatedStorage:WaitForChild('State')
        :WaitForChild('Health')
        :WaitForChild('Current') -- your current base hp
    local RSHealthMax = ReplicatedStorage:WaitForChild('State')
        :WaitForChild('Health')
        :WaitForChild('Max') -- your max hp
    local VoteGUI = LocalPlayer.PlayerGui
        :WaitForChild('ReactOverridesVote')
        :WaitForChild('Frame')
        :WaitForChild('votes')
        :WaitForChild('vote') -- it is what it is
    local MatchGui = LocalPlayer.PlayerGui
        :WaitForChild('ReactGameNewRewards')
        :WaitForChild('Frame')
        :WaitForChild('gameOver') -- end result
    ...
    task.spawn(function()
        ...
        repeat
            ...
            Success = pcall(function()
                AutoSkipCheck = (
                    LocalPlayer.PlayerGui.ReactUniversal
                        :WaitForChild('Settings')
                        :WaitForChild('window')
                        :WaitForChild('scrollingFrame')
                        :WaitForChild('Unknown')['Auto Skip'].button.toggle[1][2].Text
                    == 'Enabled'
                )
            end)
    ...
    if VoteGUI:WaitForChild('prompt').Text == 'Ready?' then --Event GameMode
        task.spawn(function()
            repeat
                task.wait()
            until StratXLibrary.Executed
            RemoteFunction:InvokeServer('Voting', 'Skip')
            prints('Ready Signal Fired')
        end)
    end
    StratXLibrary.ReadyState = false
    StratXLibrary.VoteState = VoteGUI:GetPropertyChangedSignal('Position')
        :Connect(function()
            if
                VoteGUI:WaitForChild('count').Text
                ~= `0/{#Players:GetChildren()} Required`
            then
                repeat
                    task.wait()
                until VoteGUI:WaitForChild('count').Text
                    == `0/{#Players:GetChildren()} Required`
            end
            ...
            if
                currentPrompt == 'Ready?'
                or currentPrompt == 'Skip Cutscene?'
            then --Event GameMode
                task.wait(2)
                RemoteFunction:InvokeServer('Voting', 'Skip')
    ...
    task.spawn(function()
        ...
        for i, v in next, GetTowersInfo() do
            if v.Equipped then
                StratXLibrary.TowerInfo[i] =
                    { maintab:Section(i .. ' : 0'), 0, i }
            end
        end
            ...
            while true do
                for i, v in next, Workspace.NPCs:GetChildren() do
                    if
                        ...
                        and v:FindFirstChild('HumanoidRootPart').CFrame.Y
                            > -5
                    then
                        ...
                        if UtilitiesConfig.Camera == 2 and not NPCObject then
                            ...
                            task.spawn(function()
                                repeat
                                    ...
                                    task.wait()
                                until UtilitiesConfig.Camera ~= 2
                                    or not (
                                        v:FindFirstChild('HumanoidRootPart')
                                        and v.RootPointer.Value.Parent
                                    )
                                ViewCeck = false
                                NPCObject = nil
                            end)
        ...
        function CheckReward()
            ...
            repeat
                task.wait()
            until Rewards:FindFirstChild(1) and Rewards:FindFirstChild(2) --Rewards[1] and Rewards[2]
            for i, v in ipairs(Rewards:GetChildren()) do
                if v:IsA('Frame') then
                    if
                        v:WaitForChild('content')
                            :FindFirstChild('icon')
                            :IsA('ImageLabel')
                    then
                        if
                            v:WaitForChild('content')
                                :FindFirstChild('icon').Image
                            == 'rbxassetid://5870325376'
                        then
                            RewardType = 'Coins'
        ...
        warn('Connected?')
        StratXLibrary.SignalMatchEnd = MatchGui
            ...
            :Connect(function()
                ...
                if UtilitiesConfig.Webhook.Enabled then
                    task.spawn(function()
                        loadstring(
                            game:HttpGet(
                                MainLink .. 'TDSTools/Webhook.lua',
                                true
                            )
                        )()
                        repeat
                            task.wait()
                        until type(getgenv().SendCheck) == 'table'
                ...
                if
                    not (
                        UtilitiesConfig.RestartMatch or StratXLibrary.RejoinLobby
                    )
                then
                    repeat
                        task.wait()
                    until UtilitiesConfig.RestartMatch
                        or StratXLibrary.RejoinLobby
                end
                prints(UtilitiesConfig.RestartMatch, StratXLibrary.RejoinLobby)
                prints('GameOver Changed2')
                if
                    UtilitiesConfig.RestartMatch
                    and RSHealthCurrent.Value == 0
                then --StratXLibrary.RestartCount <= UtilitiesConfig.RestartTimes
                    ...
                    repeat
                        task.wait()
                    until StratXLibrary.ReadyState
                        or (tonumber(GameWave.Text) ~= nil and tonumber(
                            GameWave.Text
                        ) <= 1)
                        or (RSHealthCurrent.Value == RSHealthMax.Value)
                    prints('Prepare Set All ListNum To 1')
                    StratXLibrary.CurrentCount = StratXLibrary.RestartCount
                    for i, v in ipairs(StratXLibrary.Strat) do
                        if
                            v.Map.Lists[#v.Map.Lists]
                            and v.Map.Lists[#v.Map.Lists].Map == RSMap.Value
                        then
                    ...
                    for i, v in
                        next,
                        StratXLibrary.Strat[StratXLibrary.Strat.ChosenID]
                    do
                        if
                            type(v) == 'table'
                            and v.ListNum
                            and type(v.ListNum) == 'number'
                        then
                            v.ListNum = 1
                        end
                    end
                    ...
                    local TimeScaleUI = LocalPlayer.PlayerGui
                        :WaitForChild('ReactUniversalHotbar')
                        :WaitForChild('Frame')
                        :WaitForChild('timescale')
                    ...
                    prints(
                        `Match {if MatchGui:WaitForChild('banner')
                                :WaitForChild('textLabel').Text
                                == 'TRIUMPH!'
                            then 'Won'
                            else 'Lose'}`
                    )
				...
				if WebSocket and WebSocket.connect then
   				...
   				until success or attemptIndex == ATTEMPT_LIMIT]]
                    if getgenv().ClassicRejoinLobby then
                        ...
                        for i, v in ipairs(StratXLibrary.Strat) do
                            local MapInStrat = v.Map.Lists[#v.Map.Lists]
                                and v.Map.Lists[#v.Map.Lists].Map
                            if table.find(SpecialMaps, MapInStrat) then
                                ...
                                local DifficultyName = v.Mode.Lists[1]
                                    and DiffTable[v.Mode.Lists[1].Name]
                                RemoteFunction:InvokeServer(
                                    'Multiplayer',
                                    'v2:start',
                                    {
...
if CheckPlace() then
    ...
    UtilitiesTab:Toggle(
        'Rejoin Lobby After Match',
        { flag = 'RejoinLobby', default = true, location = StratXLibrary }
    )
    UtilitiesTab:Toggle(
        'Show Towers Preview',
        { flag = 'TowersPreview', default = UtilitiesConfig.TowersPreview },
        function(bool)
            local TowersFolder = if bool
                then Workspace.PreviewFolder
                else ReplicatedStorage.PreviewHolder
            local ErrorsFolder = if bool
                then Workspace.PreviewErrorFolder
                else ReplicatedStorage.PreviewHolder
            for i, v in ipairs(TowersContained) do
                if v.DebugTag then
    ...
    UtilitiesTab:Button('Teleport Back To Platform', function()
        LocalPlayer.Character:FindFirstChild('HumanoidRootPart').CFrame = StratXLibrary.PlatformPart.CFrame
            + Vector3.new(0, 3.3, 0)
    end)
    local GameMode = if Workspace:FindFirstChild('IntermissionLobby')
        then 'Survival'
        else 'Hardcore'
    local Lobby = if GameMode == 'Survival'
        then 'IntermissionLobby'
        else 'HardcoreIntermissionLobby'
    UtilitiesTab:Toggle(
        'Use Timescale',
        { flag = 'UseTimeScale', default = UtilitiesConfig.UseTimeScale },
        function(bool)
            if
                (
                    bool
                    and ReplicatedStorage.State.Difficulty.Value
                        == 'Hardcore'
                )
                or (
                    bool
        ...
        task.spawn(function()
            ...
            while true do
                for Index, Object in next, Pickups:GetChildren() do
                    ...
                    if
                        Object:IsA('MeshPart')
                        and table.find(Items.Name, Object.Name)
                        and Object.CFrame.Y < 200
                    then
                        if
                            not LocalPlayer.Character:FindFirstChild(
                                'HumanoidRootPart'
                            )
                        then
                            repeat
                                task.wait()
                            until LocalPlayer.Character:FindFirstChild(
                                    'HumanoidRootPart'
                                )
                        end
                        repeat
...
UtilitiesTab:Toggle(
    'Prefer Matchmaking',
    { flag = 'PreferMatchmaking', default = UtilitiesConfig.PreferMatchmaking }
)
UtilitiesTab:Toggle(
    'Auto Skip Wave',
    { flag = 'AutoSkip', default = UtilitiesConfig.AutoSkip }
)
UtilitiesTab:Toggle(
    'Low Graphics Mode',
    { default = UtilitiesConfig.LowGraphics or false, flag = 'LowGraphics' },
    function(bool)
...
task.spawn(function()
    UI.PlayerInfo.Level =
        PlayerInfoUI:Section(`Level: {LocalPlayer:WaitForChild('Level').Value}`)
    UI.PlayerInfo.Experience = PlayerInfoUI:Section(
        `Experience: {LocalPlayer:WaitForChild('Experience').Value}`
    )
    UI.PlayerInfo.Coins =
        PlayerInfoUI:Section(`Coins: {LocalPlayer:WaitForChild('Coins').Value}`)
    UI.PlayerInfo.Gems =
        PlayerInfoUI:Section(`Gems: {LocalPlayer:WaitForChild('Gems').Value}`)
    UI.PlayerInfo.Triumphs = PlayerInfoUI:Section(
        `Wins: {LocalPlayer:WaitForChild('Triumphs').Value}`
    )
    UI.PlayerInfo.Loses =
        PlayerInfoUI:Section(`Loses: {LocalPlayer:WaitForChild('Loses').Value}`)
    UI.PlayerInfo.TimescaleTickets = PlayerInfoUI:Section(
        `Timescale Tickets: {LocalPlayer:WaitForChild('TimescaleTickets').Value}`
    )
    UI.PlayerInfo.ReviveTickets = PlayerInfoUI:Section(
        `Revive Tickets: {LocalPlayer:WaitForChild('ReviveTickets').Value}`
    )
    UI.PlayerInfo.SpinTickets = PlayerInfoUI:Section(
        `Spin Tickets: {LocalPlayer:WaitForChild('SpinTickets').Value}`
    )
...
task.spawn(function()
    ...
    if
        (
            type(getgenv().StratCreditsAuthor) == 'string'
            and #getgenv().StratCreditsAuthor > 0
        ) or type(getgenv().StratCreditsAuthor) == 'number'
    then
...
Functions.Map =
    loadstring(game:HttpGet(MainLink .. 'TDSTools/Functions/Map.lua', true))()
Functions.Loadout = loadstring(
    game:HttpGet(MainLink .. 'TDSTools/Functions/Loadout.lua', true)
)()
Functions.Mode =
    loadstring(game:HttpGet(MainLink .. 'TDSTools/Functions/Mode.lua', true))()
Functions.Place =
    loadstring(game:HttpGet(MainLink .. 'TDSTools/Functions/Place.lua', true))()
Functions.Upgrade = loadstring(
    game:HttpGet(MainLink .. 'TDSTools/Functions/Upgrade.lua', true)
)()
Functions.Sell =
    loadstring(game:HttpGet(MainLink .. 'TDSTools/Functions/Sell.lua', true))()
Functions.Skip =
    loadstring(game:HttpGet(MainLink .. 'TDSTools/Functions/Skip.lua', true))()
Functions.Ability = loadstring(
    game:HttpGet(MainLink .. 'TDSTools/Functions/Ability.lua', true)
)()
Functions.Target = loadstring(
    game:HttpGet(MainLink .. 'TDSTools/Functions/Target.lua', true)
)()
Functions.AutoChain = loadstring(
    game:HttpGet(MainLink .. 'TDSTools/Functions/AutoChain.lua', true)
)()
Functions.SellAllFarms = loadstring(
    game:HttpGet(MainLink .. 'TDSTools/Functions/SellAllFarms.lua', true)
)()
Functions.Option = loadstring(
    game:HttpGet(MainLink .. 'TDSTools/Functions/Option.lua', true)
)()
...
Functions.MatchMaking = function()
    ...
    local CanChangeMap = game:GetService('MarketplaceService')
        :UserOwnsGamePassAsync(LocalPlayer.UserId, 10518590)
    ...
    while not MapProps do
        ...
        for i, v in ipairs(StratXLibrary.Strat) do
            if
                typeof(v.Loadout.Lists[1]) ~= 'table'
                or #v.Loadout.Lists[1] == 0
            then
                continue
            end
        ...
        if not VetoUsedOnce and not CanChangeMap then
            ...
            if not CheckingForPrivateIntermission then
                ...
                local IntermissionButtons = LocalPlayer.PlayerGui
                    :WaitForChild('ReactGameIntermission')
                    :WaitForChild('Frame')
                    :WaitForChild('buttons')
                local currentVeto = IntermissionButtons:WaitForChild('veto')
                    :WaitForChild('value')
                if
                    currentVeto.Text
                    ~= `Veto ({#Players:GetChildren()}/{#Players:GetChildren()})`
                then
                    CheckingForPrivateIntermission = true
                    for i, v in ipairs(StratXLibrary.Strat) do
                        if CheckingForPrivateIntermission then
                            prints(
                                'Checking finished, Start Overriding for Map'
                            )
                            MapProps = v.Map.Lists[#v.Map.Lists]
                            Index = v.Index
                            RemoteFunction:InvokeServer(
            ...
            for i, v in next, Workspace[Lobby].Boards:GetChildren() do
                table.insert(
                    CurrentMapList,
                    v.Hitboxes.Bottom.MapDisplay.Title.Text
                )
            end
    ...
    ConsoleInfo(
        `Map Selected: {MapProps.Map}, Mode: {MapProps.Mode}, Solo Only: {MapProps.Solo}`
    )
end
...
game:GetService('CoreGui').RobloxPromptGui.promptOverlay.ChildAdded
    :Connect(function(object)
        if
            object.Name == 'ErrorPrompt'
            and object:FindFirstChild('MessageArea')
            and object.MessageArea:FindFirstChild('ErrorFrame')
        then
            TeleportService:Teleport(3260590327, LocalPlayer)
        end
    end)
        ...
        if table.find(FunctionConfig.Replace, Funcname) then
            setmetatable(t[Funcname], {
                __call = function(self, ...) --self is Functions, ...[1] is parent of self
                    local tableinfo = (select(1, ...) == t)
                            and (select(2, ...) == StratXLibrary and {
                                select(3, ...),
                            } or { select(2, ...) })
                        or { ... }
                    t[Funcname].Lists =
                        { ParametersPatch(Funcname, unpack(tableinfo)) }
                    t.Active = true
...
task.spawn(function()
    ...
    local currentVeto = IntermissionButtons:WaitForChild('veto')
        :WaitForChild('value')
    if
        UtilitiesConfig.PreferMatchmaking
        or currentVeto.Text ~= `Veto ({#Players:GetChildren()}/{#Players:GetChildren()})`
    ...
    if not StratXLibrary.Strat.ChosenID then
        ...
        repeat
            task.wait()
            for i, v in ipairs(StratXLibrary.Strat) do
                local RSMap = ReplicatedStorage:WaitForChild('State')
                    :WaitForChild('Map') --map's Name
                if
                    v.Map.Lists[#v.Map.Lists]
                    and typeof(RSMap.Value) == 'string'
                    and v.Map.Lists[#v.Map.Lists].Map == RSMap.Value
                    and not StratXLibrary.Strat.ChosenID
                then -- not apply same map dfferent mode
                    StratXLibrary.Strat.ChosenID = i
                    break
                end
            end
